...
Template['layout_header'].helpers({
  ...
  goToSend: function() {
    ...
    var accounts = PuffsAccounts.find({}).fetch();
  ...
  formattedBlockNumber: function() {
    return PuffsBlocks.latest.number > 0
      ? numeral(PuffsBlocks.latest.number).format('0,0')
      : '--';
  },
  /**
  ...
  timeSinceBlock: function() {
    if (
      PuffsBlocks.latest.timestamp == 0 ||
      typeof PuffsBlocks.latest.timestamp == 'undefined'
    )
      return false;

    var timeSince = moment(PuffsBlocks.latest.timestamp, 'X');
    var now = moment();
